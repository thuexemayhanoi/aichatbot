 ...
 *
 * Run: node tools/build-wordpress-plugin.mjs
 */
...
/** Collect every file under dir as [zipPath (posix), absPath], sorted. */
function collect(dir, base) {
  const out = [];
  for (const name of readdirSync(dir).sort()) {
    const abs = join(dir, name);
    if (statSync(abs).isDirectory()) out.push(...collect(abs, base));
    else out.push([relative(base, abs).split(sep).join('/'), abs]);
  }
  return out;
}

/** DOS date/time for the fixed timestamp 1980-01-01 00:00:00. */
...
console.log(`dist/motoai-agent.zip: ${files.length} files, ${zip.length} bytes (stored, deterministic)`);
